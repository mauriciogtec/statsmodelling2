...
& \propto \int \pi \mathrm{Dirichlet}(\pi \mid n_{1,\neg i} + \nu_1, n_{2,\neg i} +\nu_2)d\pi \\
& \propto E \left[ \mathrm{Dirichlet}(n_{1,\neg i} + \nu_1, n_{2,\neg i} +\nu_2) \right] \\
& = \frac{n_{1,\neg i} + \nu_1}{n - 1+ \nu_1 + \nu_2}.
\end{aligned}
$$
...
*Solution*. From the expression of exercise 3 it's evident that since the density splits, we can ignore the terms corresponding to $\mu$ and $\sigma^2$.

$$
\begin{aligned}
p(z_i \mid z_{\neg i}, X, \theta) &= \int\int p(z_i \mid z_{\neg i}, X, \mu,\sigma^2,\theta) d\mu d\sigma^2\\ 
& \propto \frac{n_{z_i,\neg i} + \nu_{z_i}}{n - 1+ \nu_1 + \nu_2}.
\end{aligned}
$$
...
nsim <- 1000
set.seed(110104)
prior <- list(mu0 = rnorm(K), a0 = 1, b0 = .01, kappa0 = 1., nu = rep(1, K))
...
hist(simdata, breaks = 50, prob = TRUE, xlim = c(-4.5, 4.5),
  col = alpha("blue", 0.2), border = "white", xlab = "simulated data",
  main = "Problem 7: Normal Mixture with K=5 clusters (simulated data)")
mean_dens <- numeric(grid_length)
for (k in (warmup + 1):nsim) {
  dens <- numeric(length(gridy))
  for (i in 1:length(gridy)) {
...
legend("topright", c("simdata", "posterior density", "kde estimate"),
  pch = c(15, NA, NA), lt = c(NA,  1, 1),
  col = c(alpha("blue", 0.4), "black", "magenta"))
}
```
