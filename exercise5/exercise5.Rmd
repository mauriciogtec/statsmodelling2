...
```{r}
image(as(adjmat, "Matrix"), colorkey = TRUE)
```
...
svd_output <- svd(scaled_mnist, nu = 50, nv = 50)
mnist_pca <- svd_output$u
mnist_recons <- svd_output$u %*% diag(svd_output$d[1:50]) %*% t(svd_output$v)
for (j in 1:50) {
...
K <- 10
d <- 50
prior <- list(mu0 = matrix(rnorm(K*d, 0, 0.5)), xi0 = 1, Omega0 = 1 * diag(d), kappa0 = 1, nu = rep(1, K))
```
...
image(as(adjmat[rows, rows], "Matrix"), colorkey = TRUE)
```
...
```{r}
z <- sim8$latent[nsim, ]
clust1_sample <- sample(which(z == clust), 16)
imsample <- matrix(0, 4 * 28, 4 * 28)
recons <- matrix(0, 4 * 28, 4 * 28)
for (i in 1:4) {
  for (j in 1:4) {
    obs <- mnist[clust1_sample[4*(i - 1) + j], ]
    im <- matrix(obs, ncol = 28)[ ,28:1]
    obs_recons <- mnist_recons[clust1_sample[4*(i - 1) + j], ]
...
```{r}
image(imsample, col = gray(0:32/32),  asp=1)
```

```{r}
image(recons, col = gray(0:32/32),  asp=1)
```
