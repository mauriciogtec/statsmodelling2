...
nsim <- 1000
set.seed(999)
prior <- list(mu0 = rnorm(K, 0, 3), a0 = 5, b0 = .05, kappa0 = 0.01, nu = rep(1, K))
...
simdata[1:100, ] <- rmvnorm(100, mu_true[1, ], diag(sigma_true, d))
simdata[101:200, ] <- rmvnorm(100, mu_true[2, ], diag(sigma_true, d))
simdata[201:300, ] <- rmvnorm(100, mu_true[3, ], diag(sigma_true, d))
  ...
  for (k in 1:(nsim - 1)) {
    ...
    for (i in 1:n) {
      ...
      for (j in 1:K) { 
        ...
        Sigmaj_negi <- Omegaj_negi / kappaj_negi / (xij_negi - d + 1)
          
        # assign weight
        wts[j] <-  (prior$nu[j] + nj_negi) * dmvt(Y[i, ], muj_negi, Sigmaj_negi, xij_negi - d + 1, log = FALSE)
      }
      latent[k + 1, i] <- sample(1:K, size = 1, prob = wts / sum(wts))
    }
    ...
    for (j in 1:K) {
      ...
      SSj[k + 1, j, , ] <- crossprod(Yj, Yj)
    }
     
  }
  
  list(latent = latent[-1, ], nj = nj[-1, ], sj = sj[-1, , ], SSj = SSj[-1, , , ])
}
```
...
```{r}
eval_density7c <- function(Y, sim, prior) {
  ...
  for (i in 1:n) {
    for (k in 1:nsim) {
      ...
      for (j in 1:K) { 
        ...
        xij <- prior$xi0 + nj
        Omegaj <- prior$Omega0 + SSj_negi - nj * tcrossprod(ybarj)
        Sigmaj <- Omegaj / kappaj / (xij - d + 1)
          
        # assign weight
        denominator <- denominator + nj
        wts[j] <-  (prior$nu[j] + nj) * dmvt(Y[i, ], muj, Sigmaj, xij - d + 1, log = FALSE)
      }
    dens[i] <- dens[i] +  wts[j] / denominator / nsim
    }
  }
  dens
}
...
K <- 3
d <- 2
prior <- list(mu0 = matrix(rnorm(K*d, 0, 3), ncol = d), xi0 = 20, Omega0 = 20 * diag(d), kappa0 = 0.1, nu = rep(1, K))
```
...
```{r}
gridY <- expand.grid(seq(-5, 5, length.out = 100), seq(-5, 5, length.out = 100))
densY <- eval_density7c(gridY, sim7c, prior)
```

Let's step up the plotting game an use ggplot.
...
```{r}
svd_output <- svd(scaled_mnist, nu = 50)
minst_pca <- svd_output$u
```
