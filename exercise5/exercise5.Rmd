...
```{r}
hist(resto$profit_scaled, breaks = 40)
```
...
gibbs2 <- function(nsim, y, prior) {
  ...
  sigma[1] <- 1 / omega[1] # std.dev
  mu[1, ] <- prior$mu0 + c(-sigma[1], sigma[1]) # sensible values for mu
  for (i in 1:n) {
    wts <- 0.5 * map_dbl(mu[1, ], ~ dnorm(y[i], mean = .x, sigma[1]))
    latent[1, i] <- sample(2, 1, prob = wts) # allocate ever individual
  }
  
  # constant quantities
  an <- 2*prior$a0 + 2*(n -1) / 2
  ...
  for (k in 1:(nsim - 1)) {
    ...
    kappanj <- map_dbl(1:2, ~ nj[.]  + prior$kappa0, 0)  
    ...
    # sample the means
    mu[k + 1, ] <-  map_dbl(1:2, ~ rnorm(1, munj[.], sigma[k + 1]))
  ...
  list(mu = mu, omega = omega, sigma = sigma, latent = latent)
}
```

```{r}
nsim <- 100
prior <- list(mu0 = c(0.6, 0.6), a0 = 1, b0 = 1, kappa0 = 1)
...
for (k in 1:nsim) {
  dens <- dnorm(gridy, 0.5 * sim$mu[k, 1], mu$)
  lines()
}
```
