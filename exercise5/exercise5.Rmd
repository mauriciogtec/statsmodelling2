...
Here's the modified Gibbs sampler.
...
eval_density7c <- function(Y, sim, prior, warmup = 0) {
  ...
  for (k in (warmup + 1):nsim) {
    for (i in 1:n) {
      ...
      dens[i] <- dens[i] +  sum(wts) / sum(prior$nu + sim$nj[k, ]) / (nsim - warmup)
    }
  }
  dens
}
...
**Part C: Getting tired*

The above works pretty well, but sometimes it collapses the means too often. We want to see if the less collapsed method has a slightly better performance.


```{r}
gibbs7d <- function(nsim, Y, prior, progress = FALSE) {
  # meta
  n <- nrow(Y)
  d <- ncol(Y)
  K <- length(prior$nu)
  ...
  sj <- array(0, c(nsim + 1, K, d))
  SSj <- array(0, dim = c(nsim + 1, K, d, d))
  muj <- array(0, dim = c(nsim + 1, K, d))
  Sigmaj <- array(0, dim = c(nsim + 1, K, d, d))
  ...
  for (j in 1:K) {
    ...
    sj[1, j, ] <- apply(Yj, 2, sum)
    SSj[1, j, , ] <- crossprod(Yj)
    ybarj <- sj[1, j, ] / pmax(nj[1, j], 1) 
    kappaj <-  nj[1, j]  + prior$kappa0
    muj_mean <- (nj[1, j] * ybarj + prior$kappa0 * prior$mu0[j, ]) / kappaj
    xij <- prior$xi0 +  nj[1, j] 
    Omegaj <- prior$Omega0 + SSj[1, j, , ]  - nj[1, j] * tcrossprod(ybarj) + 
      prior$kappa0 * nj[1, j] / kappaj * tcrossprod(ybarj - prior$mu0[j])
    Lambda <- drop(rWishart(1, xij,  solve(Omegaj)))
    Sigmaj[1, j, , ]  <- solve(Lambda)
    muj[1, j, ] <- rmvnorm(1, muj_mean, Sigmaj[1, j, , ] / (xij - d + 1) / kappaj)
  }
  ...
  for (k in 1:nsim) {
    ...
    for (i in 1:n) {
      ...
      for (j in 1:K) {
        # Adjust sufficient statistics if necessary
        nj_negi <- nj[k, j] - as.integer(latent[k, i] == j)
        wts[j] <-  (prior$nu[j] + nj_negi) * 
          pmin(dmvnorm(Y[i, ], muj[k, j, ], Sigmaj[k, j, , ], log = FALSE), 1)
      }
      if (sum(wts) == 0) wts <- rep(1, K)
      wts <- wts + 1e-8
      latent[k + 1, i] <- sample(1:K, size = 1, prob = wts)
    }
  
    # update sufficient statistics & resample
    for (j in 1:K) {
      # statistics
      nj[k + 1, j] <- sum(latent[k + 1, ] == j)  
      if (nj[k + 1, j] > 0) {
      ...
      # parameters
      ybarj <- sj[k + 1, j, ] / pmax(nj[k + 1, j], 1) 
      kappaj <-  nj[k + 1]  + prior$kappa0
      muj_mean <- (nj[k + 1] * ybarj + prior$kappa0 * prior$mu0[j, ]) / kappaj
      xij <- prior$xi0 +  nj[k + 1, j] 
      Omegaj <- prior$Omega0 + SSj[k + 1, j, , ]  - nj[k + 1, j] * tcrossprod(ybarj) + 
        prior$kappa0 * nj[k + 1, j] / kappaj * tcrossprod(ybarj - prior$mu0[j])
      Lambda <- drop(rWishart(1, xij,  solve(Omegaj)))
      Sigmaj[k + 1, j, , ]  <- solve(Lambda)
      muj[k + 1, j, ] <- rmvnorm(1, muj_mean, Sigmaj[k + 1, j, , ] / kappaj)
    }
    
    if(progress) pb$tick()
  }
  
  list(latent = latent, nj = nj, muj = muj, Sigmaj = Sigmaj)
}
```


```{r}
eval_density7d <- function(Y, sim, prior, warmup = 0) {
  nsim <- nrow(sim$latent)
  n <- nrow(Y)
  d <- ncol(Y)
  K <- length(prior$nu)
  ...
  for (k in (warmup + 1):nsim) {
    for (i in 1:n) {
      ...
      for (j in 1:K) { 
        ...
        nj <- sim$nj[k, j]
        muj <- sim$muj[k, j, ]
        Sigmaj <- sim$Sigmaj[k, j, , ]
        wts[j] <-  (prior$nu[j] + nj) * dmvnorm(Y[i, ], muj, Sigmaj, log = FALSE)
      }
      dens[i] <- dens[i] +  sum(wts) / sum(prior$nu + sim$nj[k, ]) / (nsim - warmup)
    }
  }
...
nsim <- 100
set.seed(999999)
sim7d <- gibbs7d(nsim, simdata, prior)
```
...
y = seq(-3, 3, length.out = resol)
grid <- data.matrix(expand.grid(x, y))
dens <- eval_density7d(grid, sim7d, prior, warmup = 50)
```
...
```{r}
adjmat <- adjacency7c(sim7d, warmup = 50)
```
...
We see that this algorithm has by far the best performance!
...
**PART A: MODEL 1 (WITH MEANS AND VARIANCES)**

Definition of prior
```{r}
K <- 10
d <- ncomp
prior <- list(mu0 = matrix(rnorm(K*d, 0, 0.5)), xi0 = d + 1, Omega0 = 0.01 * diag(d), kappa0 = 1, nu = rep(1, K))
```
...
**PART B: MODEL 2 (WITH MEANS AND VARIANCES)**

Let's try this again with the semi-collapsed method that work much better for the previous exercise.
...
K <- 10
d <- ncomp
prior <- list(mu0 = matrix(rnorm(K*d, 0, 0.5)), xi0 = d + 1, Omega0 = 0.1 * diag(d), kappa0 = 1, nu = rep(1, K))
```

```{r}
set.seed(110104)
nsim <- 100
sim8b <- gibbs7d(nsim, mnist_pca, prior)
```

```{r}
table(sim8b$latent[nsim, ])
```

```{r}
adjmat <- adjacency7d(sim8b, warmup = nsim %/% 2)
```

```{r}
rows <- sort(sample(nrow(mnist_pca), 100))
row_order <- sort(sim8b$latent[nsim, ])
image(t(adjmat[row_order, row_order]), col = gray(0:32/32), asp = 1)
```
...
```{r}
viz_cluster(6, z, r, c, mnist, mnist_recons)
```
...
```{r}
viz_cluster(7, z, r, c, mnist, mnist_recons)
```
