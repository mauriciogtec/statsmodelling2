...
Let's comparaed the observation and mean predictions now

```{r}
beta <- rstan::extract(res10, "beta")[[1]]
mu <- exp(X %*% t(beta))
mu_mean <- apply(mu, 1, mean)
mu_sd <- apply(mu, 1, sd)
mu_mode_round <- apply(round(mu), 1, function(r) {u <- unique(r); u[which.max(tabulate(match(r, u)))]})
...
```{r}
tbl = table(observed = y, prediced = mu_mode_round)[1:15, 1:15]
tbl
```
