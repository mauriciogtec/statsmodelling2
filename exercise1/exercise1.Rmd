...
```{r}
describe <- function(x) {
  data_frame(mean = mean(x), sd = sd(x), low95 = quantile(x, 0.025), up95 = quantile(x, 0.0975))
} 
params %>% 
...
```{r}
mean_lambda <- apply(res14$lambda, 2, mean)

pltdata <- data.frame(mean_lambda = mean_lambda, age = ordered(dental$age), sex = dental$Sex)
ggplot(pltdata, aes(age, mean_lambda)) +
  geom_boxplot() +
  facet_grid(. ~ sex) +
  ylab("") +
...
beta <- t(res14$beta)
err <- y - X %*% beta
mean_err <- apply(err, 1, mean)

pltdata <- data.frame(mean_err = mean_err, age = ordered(dental$age), sex = dental$Sex)
ggplot(pltdata, aes(age, mean_err)) +
  geom_boxplot() +
